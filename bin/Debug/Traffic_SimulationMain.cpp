...
Simulation::Simulation(wxWindow* parent,wxWindowID id)
{
	...
	EndText = new wxStaticText(endPanel, ID_ENDTEXT, wxEmptyString, wxPoint(145,60), wxSize(176,32), 0, _T("ID_ENDTEXT"));
...
void Simulation::OnTick( wxTimerEvent& event )
{
	switch (screenState){
        case state::runningScreen:
        {
			...
			time_per_sec++;
			if( (timeG->GetValue() >= timeG->GetRange() || score >= (0.75 * Vehicle::Gettotal()))){screenState = state::endScreen; }
        }
            break;
